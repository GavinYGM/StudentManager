...
public class Del_Items extends JPanel{
	...
	public Del_Items() {
		...
		delBtn.setBounds(180, 300,300, 100); // 保存按钮
		delBtn.setBackground(Color.CYAN);
		delBtn.setFont((new Font("宋体", Font.BOLD, 50)));
	...
	private void btn_delActionPerformed(ActionEvent e) throws IOException {
		...
		try {
			//String img1 = null;
			//img1 = f.getPath();
			//delImg.delImage(f,name);
			// 将记录保存到student表中
			
			String sql = "delete from student where name='"+name+"'";
			String sql2="delete from img where name='"+name+"'";
			int i = JDBC.executeUpdate(sql);
			int j=JDBC.executeUpdate(sql2);
			if (i == 1&&j==1) {
